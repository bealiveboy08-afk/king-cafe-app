...
export const CafeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
      ...
      if (e.key === STORAGE_KEYS.ORDERS && e.newValue) {
        try {
          ...
          if (Array.isArray(parsedOrders)) {
            setOrders((prev) => {
              // Check if a new order arrived that wasn't in previous state
              const prevIds = new Set(prev.map((o) => o.id));
              const hasNewOrder = parsedOrders.some((o) => !prevIds.has(o.id) && o.status === 'order_sent');
    ...
    // 2. Window Custom Event Listeners (immediate intra-window & custom dispatch)
    ...
    try {
      ...
      channel.onmessage = (event) => {
        if (event.data?.type === 'ORDERS_UPDATED' && Array.isArray(event.data.payload)) {
          const incomingOrders: Order[] = event.data.payload;
          setOrders((prev) => {
            const prevIds = new Set(prev.map((o) => o.id));
            const hasNew = incomingOrders.some((o) => !prevIds.has(o.id) && o.status === 'order_sent');
    ...
    // 4. Polling Fallback (ensures 100% sync reliability across sandboxed iframes and tabs)
    let lastSeenOrdersRaw = localStorage.getItem(STORAGE_KEYS.ORDERS) || '';
    const pollInterval = setInterval(() => {
      try {
        const currentRaw = localStorage.getItem(STORAGE_KEYS.ORDERS);
        if (currentRaw && currentRaw !== lastSeenOrdersRaw) {
          lastSeenOrdersRaw = currentRaw;
          const parsed = JSON.parse(currentRaw);
          if (Array.isArray(parsed)) {
            setOrders((prev) => {
              const prevIds = new Set(prev.map((o) => o.id));
              const hasNew = parsed.some((o) => !prevIds.has(o.id) && o.status === 'order_sent');
              if (hasNew && soundEnabled) {
                playNotificationSound('new_order');
              }
              return parsed;
  ...
  // Save changes to localStorage and broadcast across tabs & windows
  const persistOrders = useCallback((newOrders: Order[], newlyCreatedOrder?: Order) => {
    ...
    // 1. Dispatch custom window events for immediate local listeners
    try {
